...
### Autoencoder Training mit unauffälligen EPs & unauffälligen iFix
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
...
library(h2o)
best_model <- readRDS("best_h2o_model.rds")
...
mgrad.h2o <- readRDS("all_data.rds") |> 
  filter(ID %in% sub.mgrad$ID) |> 
  select(EP.y1:EP.y300) |>
  as.h2o()

all.h2o <- readRDS("all_data.rds") |> 
  select(EP.y1:EP.y300) |> 
  as.h2o()

reconstruction_error_mgrad <- as.data.frame(h2o.anomaly(best_model, mgrad.h2o, per_feature = FALSE))
reconstruction_error_all <- as.data.frame(h2o.anomaly(best_model, all.h2o, per_feature = FALSE))
```
...
reconstruction_error_all$Group <- "Alle" 

# Beide Datensätze kombinieren
data_combined <- rbind(reconstruction_error_unauf, reconstruction_error_mgrad, reconstruction_error_all)
...
ggplot(data_combined, aes(x = Reconstruction.MSE, color = Group, fill = Group)) +
  ...
  geom_vline(data = data.frame(
    ...
    Quantile = c(
      quantile(reconstruction_error_unauf$Reconstruction.MSE, 0.98),
      quantile(reconstruction_error_unauf$Reconstruction.MSE, 0.015),
      quantile(reconstruction_error_mgrad$Reconstruction.MSE, 0.985),
      quantile(reconstruction_error_mgrad$Reconstruction.MSE, 0.015)
    )
  ), 
...
##### Klassifizierung mithilfe der Rekonstruktionsfehler in "unauffällig", "M-Gradient" und "suspicious"
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
# Klassifizierung in "unauffällig", "M-Gradient" und "suspicous"
all_data <- all_data |> 
   mutate(
    Class = case_when(
      Reconstruction.MSE >= quantile(reconstruction_error_mgrad$Reconstruction.MSE, 0.015) &
        Reconstruction.MSE <= quantile(reconstruction_error_unauf$Reconstruction.MSE, 0.98) ~ "suspicous",
      Reconstruction.MSE > quantile(reconstruction_error_unauf$Reconstruction.MSE, 0.98) ~ "M-Gradient",
      TRUE ~ "unauffällig"
    ) |> 
    as.factor()
  ) |> 
...
### h2o-Workflow mit allen features {.justify}
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
library(h2o)
...
hyper_params <- list(
  ...
  activation = c("Tanh"),            # Verschiedene Aktivierungsfunktionen
  l1 = c(1e-5, 1e-6),                                         # L1-Regularisierung
  epochs = c(50#, 100
...
saveRDS(best_model, "allf_best_h2o_model.rds")
...
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
#| label: fig-h2o-model_summaryII
#| fig-cap: Zusammenfassung des besten h2o-Modells
library(h2o)
best_model <- readRDS("allf_best_h2o_model.rds")
...
#### Schwellenwert für die Anomalieerkennung festlegen
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
#| label: fig-allf-reconstruction_error
#| fig-cap: unauffällige ChromatogrammeRekonstruktionsfehler mit y, m und fdd und 97.5%-Quantil
...
##### Schwellenwert für die Anomalieerkennung festlegen
```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
#| label: fig-reconstruction_error_mgradII
#| fig-cap: Rekonstruktionsfehler für M-Gradienten und 97.5%-Quantil
...
ggplot(data_combined, aes(x = Reconstruction.MSE, color = Group, fill = Group)) +
  ...
  geom_vline(data = data.frame(
    Group = c("Unauffällig", "Unauffällig", "M-Gradient", "M-Gradient"),
    Quantile = c(
      quantile(reconstruction_error_unauf$Reconstruction.MSE, 0.985),
      quantile(reconstruction_error_unauf$Reconstruction.MSE, 0.015),
      quantile(reconstruction_error_mgrad$Reconstruction.MSE, 0.985),
      quantile(reconstruction_error_mgrad$Reconstruction.MSE, 0.015)
    )
...
##### Klassifizierung mithilfe der Rekonstruktionsfehler in "unauffällig", "M-Gradient" und "suspicious" nach training mit allen features
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
# Klassifizierung in "unauffällig", "M-Gradient" und "suspicous"
all_data <- readRDS( "all_data.rds" ) |> 
   mutate(
    ClassAll = case_when(
      Reconstruction.MSE >= quantile(reconstruction_error_mgrad$Reconstruction.MSE, 0.015) &
        Reconstruction.MSE <= quantile(reconstruction_error_unauf$Reconstruction.MSE, 0.985) ~ "suspicous",
      Reconstruction.MSE > quantile(reconstruction_error_unauf$Reconstruction.MSE, 0.985) ~ "M-Gradient",
      TRUE ~ "unauffällig"
    ) |> 
    as.factor()
  ) |> 
  select(ClassAll, everything())
```

### Vergleich der Klassifizierungen {.justify}

```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
#| label: tbl-class_comparison
#| tbl-cap: Vergleich der Klassifizierungen (autencoder mit y)

class_comparison <- readRDS("all_data_mgrad_validation.rds") |> 
  select(Class) |> 
  count(Class) |> 
  mutate(Percentage = round(n / sum(n) * 100, 0)) |>  # Berechnung der relativen Häufigkeit
  flextable() |>
  width(j = 1, width = 3.5)
class_comparison
...
```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
#| label: tbl-class_comparison-allf
#| tbl-cap: Vergleich der Klassifizierungen (autoencoder mit allen features)

class_comparison <- readRDS("allf_data_mgrad_validation.rds") |> 
  select(ClassAll) |> 
  count(ClassAll) |> 
  mutate(Percentage = round(n / sum(n) * 100, 0)) |>  # Berechnung der relativen Häufigkeit
  flextable() |>
...
```
```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
#| label: tbl-class_comparison-gpt
#| tbl-cap: Vergleich der Klassifizierungen (gpt)


class_comparison <- readRDS("GPT_validation.rds") |> 
  select(Classification) |> 
  count(Classification) |> 
  mutate(Percentage = round(n / sum(n) * 100, 0)) |>  # Berechnung der relativen Häufigkeit
  flextable() |>
  width(j = 1, width = 3.5) #|> 
  #colformat_num(j = "Percentage", digits = 2) |> # Formatierung der Prozentwerte
  #set_header_labels(n = "Count", Percentage = "Percentage (%)") # Spaltenüberschriften anpassen
...
### Trainingsdaten mgrad speichern
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
# Trainingsdaten speichern
all.data.mgrad.split <- all_data |> 
  select(ID, Class, EP.y1:fdd300) |>
  initial_validation_split(prop = c(0.6, 0.2), strata = Class)

saveRDS(all.data.mgrad.split, "all_data_mgrad_split.rds")

all.data.mgrad.training <- training(all.data.mgrad.split)
saveRDS(all.data.mgrad.training, "all_data_mgrad_training.rds")

all.data.mgrad.validation <- validation(all.data.mgrad.split)
saveRDS(all.data.mgrad.validation, "all_data_mgrad_validation.rds")

```

### Trainingsdaten mgrad mit allen features speichern
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
# Trainingsdaten speichern
all.data.mgrad.split <- all_data |> 
  select(ID, ClassAll, EP.y1:fdd300) |>
  initial_validation_split(prop = c(0.6, 0.2), strata = ClassAll)

saveRDS(all.data.mgrad.split, "allf_data_mgrad_split.rds")

all.data.mgrad.training <- training(all.data.mgrad.split)
saveRDS(all.data.mgrad.training, "allf_data_mgrad_training.rds")

all.data.mgrad.validation <- validation(all.data.mgrad.split)
saveRDS(all.data.mgrad.validation, "allf_data_mgrad_validation.rds")
