...
Beim tokenisieren wird der Befundtext in einzelne Wörter zerlegt, alles auf Kleinbuchstaben umgewandelt und die Wörter als Tri-Grame abgespeichert.<br>
Die Trigrame geben den Kontext der Wörter wieder und können als Features für die Klassifizierung verwendet werden.
...
#### Usupervised Clustering mit tidymodeling, tidytext und K-Means {.justify}
...
```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
set.seed(8173)
wss <- map_dbl(1:10, ~ kmeans(tri.grams.matrix, centers = ., nstart = 25)$tot.withinss)
```
...
# Plot der Ergebnisse
ggplot(results, aes(x = nstart, y = tot_withinss)) +
  geom_line() +
  geom_point() +
  labs(title = "",
       x = "nstart",
...
hex.string <- EPcsv1$curve[EPcsv1$ID == 7970]
...
#### Eintragen der decimalen Chromatogramm-Daten EPcsv1
```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
...
#### Entwicklung der Features für die Modellierung
```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
hex.string <- EPcsv1$curve[EPcsv1$ID == 1675]

# Convert each hex part to decimal
EP.dec.y <- substring(hex.string, seq(1, nchar(hex.string), 4), seq(4, nchar(hex.string), 4)) |> 
  sapply(function(x) as.integer(paste0("0x", x)))

# dataframe with EP elution time points, decimal EP signal values, slopes, second derivatives, and AUC
...
dat <- dat |>
  mutate(m = ((lead(EP.y) - EP.y / (lead(EP.t) - EP.t)) |> replace_na(0)),
         ...
         AUC =pmap_dbl(
           ...
           ~ ifelse(
             is.na(..1) | is.na(..3), 0,  # Prüft, ob genügend Punkte vorhanden sind
...
dev.df1 <- EPcsv1[5000:10000,] |> # Kopieren der ersten 50 Zeilen von EPcsv1 in dev.df1
...
num_points <- 293 # Anzahl von EP.y-Werten
...
#| tbl-cap: EP-Daten mit Klassifikation und Peakdaten
...
### Testen der Chromatogramm-Daten Einträge
```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
# Testen der Chromatogramm-Daten Einträge
# Select the columns of interest
peak_columns <- dev.df  |> 
  select(starts_with("peak") & ends_with("height")) |>   # Adjust this if column names vary
  select(-contains("peaks"))  # Remove position columns

# Check for values between 80 and 200 in each column
peaks_in_range <- peak_columns  |> 
  summarise(across(everything(), ~ any(. >= 80 & . <= 200)))

# Output column names that have values in the range
columns_with_values <- names(peaks_in_range)[peaks_in_range]
...
Somit werden pro Chromatogramm 292 Flächen (AUCs) als features berechnet.
