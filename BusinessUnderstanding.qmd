...
[soviele Merkmale wie möglich berechnen? oder sich von vorneherein beschränken?]{style="color:#00008B;font-weight:bold"}
...
[soviele zusätzlich vorhandenen Analysen-Resultate pro Chromatogramm wie möglich mit speichern? oder sich von vorneherein beschränken? Siehe unten DAG...]{style="color:#00008B;font-weight:bold"}
...
[**oder besser ein DAG? Vielleicht ist dann eine PCA gar nicht nötig?**]{style="color:#00008B"}
[Link to DataPreparation](DataPreparation.qmd#var_selection)
-   Die Interaktionen & Colinearitäten zwischen den Merkmalen wird untersucht und berücksichtigt
...
[glm, gam, XGBoost, random forrest, tensorflow(?), andere an die ich denken sollte?]{style="color:#00008B;font-weight:bold"}
...
pplan <- ganttAddTask(pplan, "transpose hexa into dec", "2024-12-06", "2024-12-07")
pplan <- ganttAddTask(pplan, "prepare new chrom.data (peak, area...)", "2024-12-10", "2024-12-15")
pplan <- ganttAddTask(pplan, "prepare training and validation data sets", "2024-12-16", "2024-12-17")
pplan <- ganttAddTask(pplan, "DAG?", "2024-12-18", "2024-12-19", done = 95)
pplan <- ganttAddTask(pplan, "variable selection", "2024-12-19", "2024-12-20", done = 60)
pplan <- ganttAddTask(pplan, "Modeling")
pplan <- ganttAddTask(pplan, "glm/gam", "2024-12-20", "2024-12-21", done = 0)
pplan <- ganttAddTask(pplan, "random forrest", "2024-12-20", "2024-12-21", done = 0)

pplan <- ganttAddTask(pplan, "Evaluation", "2025-01-15", "2025-01-30", done = 0)
